...
void optimization_@VAR@() {
    TFile *file_data = TFile::Open("/user/u/u25lekai/work/analysis_X/TMVA/sideband_PSI_BDT_test4.root");
    TTree *tree_data = (TTree*)file_data->Get("tree");

    TFile *file_MC = TFile::Open("/user/u/u25lekai/work/analysis_X/TMVA/MC_PSI2S_BDT_test4.root");
    ...
    TString flag = "";
    Double_t fb = 0.549496, fs = 0.2606908;
    ...
    c1->SaveAs(Form("./FOM/%s_%d_test4.pdf", var_name.Data(), nbin));

    std::ofstream fout("FOM/FOM_summary_test4.txt", std::ios::app); 
    if (fout.is_open()) {
        if (flag == "g"){
            fout
