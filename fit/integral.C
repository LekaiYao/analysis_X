...
void integral(){
    RooRealVar B_mass("B_mass", "B_mass", 3.6, 4.0);
    RooRealVar c1("c1", "coefficient #1", 0.0690794);
    RooRealVar c2("c2", "coefficient #2", -0.0923996);
    RooChebychev bkg("bkg", "Chebychev background", B_mass, RooArgList(c1, c2));
    
    // Assume B_mass is an externally defined RooRealVar
    B_mass.setRange("fullregion", 3.6, 4.0);
    ...
    std::cout << "fb for X(3872) is: " << value_X/(value_full - value_PSI - value_X) << std::endl;
}
